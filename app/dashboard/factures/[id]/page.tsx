...
export default function ModifierFacture() {
  ...
  const [form, setForm] = useState({
    client_id: '', titre: '', date_echeance: '',
    tva_taux: 20, notes: '', statut: 'en_attente'
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('factures').update({
      ...
      lignes, sous_total: sousTotal,
      tva_taux: form.tva_taux,
      tva_montant: tvaMontant,
      total, notes: form.notes, statut: form.statut
    }).eq('id', id)
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <h1 className="text-2xl font-bold mb-8">Modifier la facture</h1>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.tva_taux} onChange={e => setForm({...form, tva_taux: Number(e.target.value)})}
                  className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-violet-500 transition">
              ...
              {lignes.map((ligne, index) => (
                <div key={index} className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-5">
                    <input type="text" value={ligne.description} onChange={e => updateLigne(index, 'description', e.target.value)}
                  ...
                  <div className="col-span-2">
                    <input type="number" value={ligne.quantite} onChange={e => updateLigne(index, 'quantite', Number(e.target.value))}
                  ...
                  <div className="col-span-2">
                    <input type="number" value={ligne.prix_unitaire} onChange={e => updateLigne(index, 'prix_unitaire', Number(e.target.value))}
                      className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 text-sm text-center focus:outline-none focus:border-violet-500 transition" />
                  </div>
                  <div className="col-span-2 text-center text-white font-medium text-sm">{ligne.total.toFixed(2)} €</div>
                  <div className="col-span-1 text-center">
                    {lignes.length > 1 && (
                      <button type="button" onClick={() => setLignes(lignes.filter((_, i) => i !== index))} className="text-red-400 hover:text-red-300">✕</button>
                    )}
                  </div>
                </div>
              ))}
            </div>
            <button type="button" onClick={() => setLignes([...lignes, { description: '', quantite: 1, prix_unitaire: 0, total: 0 }])}
              className="mt-4 text-violet-400 hover:text-violet-300 text-sm transition">+ Ajouter une ligne</button>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <div className="max-w-xs ml-auto space-y-2">
              <div className="flex justify-between text-sm"><span className="text-slate-400">Sous-total HT</span><span>{sousTotal.toFixed(2)} €</span></div>
              <div className="flex justify-between text-sm"><span className="text-slate-400">TVA ({form.tva_taux}%)</span><span>{tvaMontant.toFixed(2)} €</span></div>
              <div className="flex justify-between font-bold text-lg border-t border-slate-700 pt-2"><span>Total TTC</span><span className="text-violet-400">{total.toFixed(2)} €</span></div>
            </div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <h2 className="font-semibold mb-4 text-violet-400">📝 Notes</h2>
            <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={3}
              className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-violet-500 transition resize-none" />
          </div>
          {message && (
          ...
          <div className="flex gap-3">
            ...
            <a href="/dashboard/factures">
              <button type="button" className="border border-slate-700 hover:border-slate-500 text-white px-8 py-3 rounded-xl transition">Annuler</button>
            </a>
          </div>
        </form>
      </div>
